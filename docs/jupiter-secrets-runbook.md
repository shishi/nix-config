...
やり直す前に、3 ファイルが同じ commit に保存済みで、管理用 age 鍵のバックアップが
あることを確認する。誤って単体で削除した場合も、3 ファイルすべてを同じ commit から
復元し、異なる世代を混在させない。
...
`/var/lib/sops-nix/key.txt` では新しい `runtime.yaml` を復号できない。この操作は
...
古い管理用鍵のバックアップがなければ、古い `bootstrap.yaml` は復号できなくなる。
新しい管理用鍵はパスワードマネージャーへ改めてバックアップする。

管理用鍵だけを削除しても再初期化にはならない。既存の暗号文を維持するなら
バックアップから同じ鍵を復元し、作り直すなら暗号文 3 ファイルも一緒に削除する。
個別の secret だけを削除する操作には対応しない。不要になった値は「既存値の更新」で
別の値へ置き換える。

この再作成は、漏えいした資格情報の失効処理ではない。Git 履歴の古い暗号文と古い鍵を
持つ主体は古い値を復号できる。漏えい対応では各 secret 自体も変更し、Tailscale OAuth
client などの外部資格情報を提供元で失効させて再発行する。
...
暗号化済みファイルの一部だけが存在する場合は、3 ファイルすべてを同じ commit から
