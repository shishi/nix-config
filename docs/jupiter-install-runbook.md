# jupiter: NixOS 初期インストール runbook

## 0. 目的と完了条件

未インストールの Jupiter に NixOS を配置し、LUKS で暗号化されたシステムを
初回起動できる状態にする。Secure Boot の鍵と initrd SSH host key は
インストール中に作成するが、Secure Boot の有効化と TPM2 enroll は扱わない。

この runbook の完了後は
[Secure Boot／TPM2 runbook](jupiter-secure-boot-runbook.md)へ進む。
secret の作成と更新は
[Jupiter secrets runbook](jupiter-secrets-runbook.md)を使う。
...
- ディスクは `nvme0n1`(KINGSTON OM8PGP41024Q-A0 / 953.9 GB)1 本。
  `hosts/jupiter/disko.nix` の `device` と一致している。**§2.3 の disko phase は
...
([Secure Boot／TPM2 runbook §6](jupiter-secure-boot-runbook.md) が挙げる操作)の
...
この runbook は、対象マシンにまだ NixOS がインストールされていない場合だけ使う。
...
([Secure Boot／TPM2 runbook §2.1・§2.2](jupiter-secure-boot-runbook.md))は、
...
[secrets runbook](jupiter-secrets-runbook.md) に従い、暗号化済みの
`.sops.yaml`、`secrets/bootstrap.yaml`、`secrets/runtime.yaml` と、
Git 管理対象外の管理用 age 鍵を準備する。3 つの暗号化済みファイルを
同じ commit に含めてから次へ進む。

### 2.2 installer を起動して root に鍵を置く
...
パスフレーズ入力、Secure Boot／TPM2 runbook §2.5・§2.7 の firmware 操作には
別途コンソールが要る。
...
installer へ繋ぐ `ssh` すべてに付ける**(§2.2〜§2.5 の各コマンドにも書いてある)。
ISO の `/etc/ssh` は tmpfs で **installer の host key は起動のたびに作り直される**
ため、`known_hosts` に記録すると installer の 2 回目の起動でも、インストール後の
...
[Secure Boot／TPM2 runbook §5.2](jupiter-secure-boot-runbook.md) の照合に使う。
...
消去済み OS の boot entry は起動に必須ではない。削除する場合は、表示された
ラベルと対象ディスクを照合してから、実際の boot ID を `efibootmgr -b <ID> -B`
へ渡す。過去の実測 ID を再利用しない。

## 3. 初回起動後の確認

初回起動では TPM2 がまだ未 enroll なので、コンソールで LUKS パスフレーズを
聞かれる。secrets runbook で登録した値を使う。
...
インストール時に sbctl 鍵と initrd SSH host key を作成し、Lanzaboote まで
適用している。この経路では、[Secure Boot／TPM2 runbook](jupiter-secure-boot-runbook.md)
の鍵生成と `nixos-rebuild switch` を繰り返さない。

初回起動時点では Secure Boot は無効で、TPM2 も未 enroll である。
LUKS パスフレーズで起動した後、Secure Boot／TPM2 runbook の
「Setup Mode に入る」から続ける。既に Setup Mode の場合は、PK を消去せず
鍵の enroll へ進む。

## 5. 検証済み範囲と制限

phase 分割と target 上での鍵生成は VirtualBox VM で確認済みである。
SOPS を使う wrapper は fixture で自動テストしている。
現行 wrapper による実機の `nixos-anywhere` は未確認である。

実機では `<target>` と `<port>` を実際の値へ置き換える。
installer 起動前は Secure Boot を無効にする。firmware メニューの操作は
ベンダー依存であり、この runbook では確認していない。
