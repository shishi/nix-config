...
秘密の準備(§2.1)を除くと、人間の操作は次の 3 か所だけで、間はすべて
`jupiter-install` が自動で進める。ラッパーも操作が要る箇所では `[操作]` を
先頭に付けた案内を出すので、画面の案内とこの一覧は一致する。

1. **コンソール**(§2.2): インストーラーを起動し、`sudo passwd root` と IP の確認
2. **ワークステーション**(§2.3): `nix run .#jupiter-install ...` を実行し、
   直後に root パスワードを 1 回入力する。以降、完了表示まで操作は無い
3. **完了表示**(§2.3〜§2.4): fingerprint の保存、配送一覧の照合、
   表示された停止コマンドの実行
...
接続先は常に `root@` である。disko と対象機上の鍵生成が root 権限を要るためで、
`nixos` ユーザーで繋いで `sudo` する形は採らない(人間の手順は減らず、
遠隔コマンドすべてに sudo の配管が増えるだけ)。ラッパーも `root@` 以外を拒否する。
