...
| Docker CE(unix socket のみ。TCP 公開は #38 裁定で削除) | 同上 | nixos/docker.nix |
| locale ja_JP.UTF-8 | 同上 | nixos/locale.nix |
| fish が login shell(chsh) | 手動 | nixos/users.nix |
| fish の PATH 定義(nix profile → ~/.cargo/bin の順) | **dotfiles repo**(#8(b)) | 同じ dotfiles を展開 |
| 日本語入力 | Windows 側 IME(WSL 内 fcitx5 は導入しない — #37) | nixos/input-method.nix(fcitx5+skk) |
